...
- 日本時間 8:00 に、削除前に指定ログチャンネルへテキストメッセージ数を投稿
...
ログ投稿先は `1543273845257928747` です。集計対象は削除直前の深夜限定テキストチャンネルに存在する全メッセージで、作成時の通知メッセージも含みます。
メッセージ数の取得またはログ投稿に失敗した場合も、エラーをWorkerログへ記録してチャンネル削除は継続します。
